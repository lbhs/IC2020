    ...
    void OnTriggerEnter2D(Collider2D collider)
    {
        ...
        else if (DotCount == 1)
        {
            ...
            if (collider.transform.root.gameObject.tag == "Diatomic")  //This section breaks a diatomic element into two atoms
            {
                ...
                if (Diatomic.GetComponent<DiatomicScript>().BondDissociationEnergy > DisplayJoules.JouleTotal)
                {
                    GameObject.Find("NotEnoughJoulesDisplay").GetComponent<CannotBreakBond>().Denied();
                    //NotEnoughJoules.text = "You don't have enough joules to break this bond";
                    return;
                }
                DisplayJoules.JouleTotal -= Diatomic.GetComponent<DiatomicScript>().BondDissociationEnergy;
                ...
                return;  //return???  LOOK THIS OVER--MAYBE ALL IS DONE AND SHOULDN'T PROCEED WITH ANY OTHER SCRIPTS
            }
            ...
            else if (DotCount == 1)
            {
                ...
                if (Atom1 != Atom2)
                {
                    ...
                    {
                        GameObject.Find("NotEnoughJoulesDisplay").GetComponent<CannotBreakBond>().Denied(); //prints "You don't have enough joules to break this bond";
                        return;
                    }
                    ...
                    if (Atom1.tag == "Hydrogen" || Atom1.tag == "Chlorine")
                    {
                        ...
                        //CALL MCTOKEN FUNCTION HERE!!!  REMOVE THE MCTOKEN AND SUBTRACT POINTS WHEN A MOLECULE CONTAINING ONLY H AND CL IS DISSOCIATED
                    ...
                    if (Atom2.tag == "Hydrogen" || Atom1.tag == "Chlorine")
                    {
                        ...
                        //THE SCRIPT BELOW MAKES ATOM1 "UNBONDED" IF IT IS AN OXYGEN OR CARBON THAT IS ALL ALONE--THIS WORKS (I think)
                    ...
                    if (Atom2.tag != "Hydrogen" && Atom1.tag != "Hydrogen")  //Unbonding Carbons is Complicated!!!!
                    {
                        ...
                        if (AtomInventory.MoleculeList[MolIDValue].Count == 2) //3  && AtomInventory.MoleculeList[MolIDValue].Contains(GameObject.Find("MCToken2")))
                        {
                            print("hello world");
                            ...
                            //Decrease Joules by bond dissociation energy value
                        ...
                        {     
                            ...
                            if (Atom1.GetComponent<FixedJoint2D>())   //trying to find the joint that links the two carbons we are unbonding
                            {
                                ...
                                print("Atom 1 joints:");
                                //NEED TO RESET ATOM1 MOLECULEID TO ZERO IF IT IS THE ONLY ATOM IN ITS MOLECULE ID AFTER DISSOCIATION
                            ...
                            else                //if Atom2 has bonds, add the attached atoms to BondingPartnerList
                            {
                                ...
                                foreach (FixedJoint2D jointToBreak in JointArray)
                                {
                                    ...
                                    //NEED TO CHECK ATOM1 TO SEE IF IT IS ALL ALONE--IF SO, SET MOLECULE ID TO ZERO!!!!!
                                }
                            }
                            //Set Atom2 to Unbonded and MolID Zero if it is all alone
                            ...
                            foreach (GameObject atom in AtomInventory.MoleculeList[MolIDValue])  //This is to move the hydrogens attached to carbons
                            {
                                ...
                                {
                                    print("this atom has a FixedJoint" + atom);   //for debugging purposes
                            ...
                            if (AtomInventory.MoleculeList[MolIDValue].Count == 1)  //check to see if Atom1 is all alone, if so, MoleculeID = zero
                            {
                                print("Atom1 gets reset to MolID zero");  //THIS ISN'T WORKING FOR OXYGEN-OXYGEN BONDS. . . 
                                Atom1.GetComponent<BondMaker>().bonded = false;
                                Atom1.GetComponent<BondMaker>().MoleculeID = 0;
                                AtomInventory.MoleculeList[MolIDValue].Clear();
                            }
